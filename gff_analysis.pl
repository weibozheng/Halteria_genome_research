use Bio::SeqIO;
open(out1,'>C:\Users\wbzhe\Stentor\Stentor_intronfree_denovo_CDS_v1.fa');
open(out2,'>C:\Users\wbzhe\Stentor\Stentor_intronfree_denovo_protein_v1.fa');
open(out3,'>C:\Users\wbzhe\Stentor\Stentor_intronfree_denovo_intron_v1.fa');
my $stream_Trans = Bio::SeqIO->new(-format => 'fasta',-file   => 'C:\Users\wbzhe\Stentor\Stentor_genome_SPCA_v4_intronfree.fasta');
while(my $result_Trans=$stream_Trans->next_seq()){
	$idthis=$result_Trans->id();
	$seq{$idthis}=$result_Trans->seq();
}
local $/='# end gene';
open(infile,'C:\Users\wbzhe\Stentor\Stentor_intronfree_v1.gff');
@data=<infile>;
#print "$data[1]";
